...
pub fn create_builder(input: TokenStream) -> TokenStream {
    ...
    if let Body::Struct(s) = item.body {
        ...
        let validator = get_validator(&item.attrs);
        ...
        let parsed: String = if let Some((validator, error)) = validator {
...
/// Gets validator function and error type based on attribute if there is one.
fn get_validator(attrs: &[Attribute]) -> Option<(Path, Option<Path>)>  {
    let mut iter = attrs.iter()
        .filter_map(|a| {
            if let MetaItem::List(ref name, ref value) = a.value {
    ...
    if let Some(r) = result {
        ...
        Some((v.expect("Validator function has to be provided for `builder_validate` attribute."), e))
    } else {
        None
    }
}
